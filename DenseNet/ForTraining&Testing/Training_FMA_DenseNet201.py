...
def train_model(model, train_gen, val_gen, train_steps, val_steps, epochs, fold):
    ...
    with open('./modelNewDenseNet201.txt', 'w') as f:
        with redirect_stdout(f):
            model.summary()
    model.summary()
    file_txt = working_dir + "time201.txt"
...
for train_idx, val_idx in kfold.split(train_data):
    ...
    # Using DenseNet
    base_model_densenet = DenseNet201(include_top=False, weights='imagenet', input_shape=(224, 224, 3))
    ...
    model_dense_201 = Model(inputs=base_model_densenet.input, outputs=headModel)
    ...
    train_model(model_dense_201, train_generator, val_generator, train_steps, val_steps, nepoch, fold)
    print('========='*10)
